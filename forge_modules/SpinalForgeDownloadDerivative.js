...
function SpinalForgeDownloadDerivative(
  model,
  BUCKET_KEY,
  file_name,
  spinalForgeAuth
  ...
  this.getManifest = function(oAuth, urn) {
    ...
    if (urn === undefined || urn === null)
      return Promise.reject(
        "Missing the required parameter 'urn' when calling getManifest"
      );
    ...
    return new Promise(function(fulfill) {
      ...
      _self.listAllDerivativeFiles(bubble, function(error, result) {
        ...
        console.log(
          ...
          0 | (result.totalSize / (1024 * 1024)),
          "MB"
        );
        console.log("\n\nDownloading derivative files\n\n");
        console.log(result);
        _self.downloadAllDerivativeFiles(result.list, _self._outPath, function(
          failed,
          succeeded
        ) {
          _self.failed = failed;
  ...
  this.downloadAllDerivativeFiles = function(fileList, destDir, callback) {
    ...
    var downloadOneItem = function() {
      ...
      var downloadComplete = function(error) {
        done++;
        if (error) {
          failed++;
          console.error(
        ...
        console.log(
          "Progress:",
          (100 * (failed + succeeded) / flatList.length) | 0,
          "%"
        );
        if (done === flatList.length) callback(failed, succeeded);
        else setTimeout(downloadOneItem, 0);
      };
    ...
    for (var k = 0; k < 10; k++) downloadOneItem();
  };
  ...
  this.getThumbnail = function(urn, guid, sz, outFile, callback) {
    ...
    if (urn === undefined || urn === null)
      return Promise.reject(
        "Missing the required parameter 'urn' when calling getThumbnail"
      );
    var queryParams = {
      width: sz,
      height: sz,
            ...
            if (success) {
              ...
              if (manifest && manifest.assets) {
                for (var j = 0; j < manifest.assets.length; j++) {
                  ...
                  // Those are listed explicitly in the bubble as property database role
                  // so we will get them anyway
                  if (asset.URI.indexOf("../") === 0) continue;
                  estSize += asset.size;
                  myItem.files.push(asset.URI);
                }
              }
  ...
  this.traverse_node = function(node, parent, res, bubble) {
    ...
    if (
      ...
      res.push(item);
      if (
        node.mime == "application/autodesk-svf" ||
  ...
  this.get_forge_models = function() {
    console.log(
      "Starting to Download the file svf in forge and upload it to the SpinalCore."
    );
    _self.run();
  };
}
